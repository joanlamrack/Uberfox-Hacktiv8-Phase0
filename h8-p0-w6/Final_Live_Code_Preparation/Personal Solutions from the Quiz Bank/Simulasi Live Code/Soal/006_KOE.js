...
function letterRemoverRecursive(sentence, letter) {
	if(sentence.length === 1 && sentence[0] !== letter){
		return sentence[0];
	}
	else if(sentence.length === 1 && sentence[0] === letter){
		return "";
	}
	else{
		return (sentence[0] !== letter ? sentence[0]:"") + letterRemoverRecursive(sentence.slice(1), letter);
	}
}
