...
/*
  ...
  SET ugleNumbers WITH ARRAY CONTAINING 2,3,5 
  SET output WITH EMPTY ARRAY

  FOR LOOP FROM 2 UNTIL num, INCREMENT EVERYTIME
    FOR EACH ugNum of uglyNumbers
      IF i MOD ugNum EQUAL 0
        APPEND i TO output 
        BREAK
      ENDIF
    ENDFOR
...
function getUglyNumber(num) {
  var uglyNumbers = [2,3,5];
  var output = [];

  for(var i = 2 ; i <= num ; i++){
    for(var ugNum of uglyNumbers){
      if(!(i % ugNum)){
        output.push(i);
        break;
      }
