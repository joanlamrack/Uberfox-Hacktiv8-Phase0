...
function thisStringIsPossible(str_param, str){
	str = Array.from(str);
	for(var letter_param of str_param){
		if(str.indexOf(letter_param) !== -1){
			delete str[str.indexOf(letter_param)]
		}
		else{
			return false;
		}
...
function decypher(code){
	...
	for(var code_item in table){
		var searching_possible = thisStringIsPossible(code_item, code);
		if(searching_possible[0] && searching_possible[1].length===0){
			return table[code_item];
		}else if(searching_possible[0] && searching_possible[1].length > 2){
			return table[code_item] + decypher(searching_possible[1]);
		}
	}
}

function nextTargetArea(code) {
	return "District "+decypher(code)+ " is the next target!";
}
