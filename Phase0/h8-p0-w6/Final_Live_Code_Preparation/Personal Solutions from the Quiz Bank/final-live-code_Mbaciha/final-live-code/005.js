...
function trainSeating (penumpang) {
  var output = {}
  for(var seater of penumpang){
    if(!output[seater.gerbong]){
      output[seater.gerbong] = [{nama : seater.nama, seat : seater.seat}];
    }
    else{
      output[seater.gerbong].push({nama : seater.nama, seat : seater.seat});
    }
  }
  return output;
}
