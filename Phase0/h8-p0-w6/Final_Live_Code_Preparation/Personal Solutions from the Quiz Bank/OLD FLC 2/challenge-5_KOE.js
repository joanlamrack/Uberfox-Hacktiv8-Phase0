...
function thisStringIsPossible(possibleStr, strInput){
	strInput = Array.from(strInput);
	for(var char of possibleStr){
		if(strInput.indexOf(char) !== -1){
			delete strInput[strInput.indexOf(char)];
		}
		else{
			return false;
		}
	}
	return [true, strInput.filter(x => x)]
}
...
function crackingTheCode(code){
	...
	for(var num in table){
		var searchIsPossible = thisStringIsPossible(num, code)
		if(searchIsPossible[0] && searchIsPossible[1].length === 0){
			return table[num];
		}
		else if(searchIsPossible[0] && searchIsPossible[1].length > 0){
			return table[num] + crackingTheCode(searchIsPossible[1]);
		}
	}
}

function nextTargetArea(code) {
	return crackingTheCode(code);
}
...
console.log(nextTargetArea('TWOTWOTHREEEIGHTEIGHTTHREE')); // "District 26 is the next target!"